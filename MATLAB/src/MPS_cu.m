function [x] = MPS_cu(sdf, grid, varargin)
    ...
    % 2-Probabilistic Primitive Marching---------------------------------------  
        ...
        for i = 1 : num_region
            ...
            while ~all(valid)
                ...
                [x_temp(i, :), occ_idx, valid, num_idx(i, :)] = fitCuboidTSDF( ...
                    sdf(roi(i).idx), ...
                    x_init, ...
                    grid.truncation, ...
    ...
    function [x, occ_idx, valid, num_idx] = fitCuboidTSDF(...
    ...
    for iter = 1 : para.maxIter    
        ...
        % calculating the signed distance of voxels to the current cuboid
        sdf_current = sdfCuboid(x, points, 0);
        % find the voxels activated by the current cuboid configuration
        ...
        cost_func = @(para) differenceCuSDF(...
        ...
        if relative_cost < para.switch_tolerance && iter ~= 1 ...
            ...
            scale_idx = find(and(scale_ratio > 0.8, scale_ratio < 1.2));
    ...
    %--------------------------------------------------------------------------
        function [value] = cost_switched(para, sdf, points, truncation, weight)
            ...
            for i = 1 : size(para, 1)
                value(i, 1) = sum(differenceCuSDF(para(i, :), ...
                    sdf, points, truncation, weight) .^ 2, 2);
            end
        end
    %--------------------------------------------------------------------------
        function [dist] = differenceCuSDF(para, sdf, points, truncation, weight)
            
            R = eul2rotm(para(6 : 8));
            t = para(9 : 11);
            X = R' * points - R' * t';
            
            dx = max(abs(X(1, :)) - para(3), 0);
            dy = max(abs(X(2, :)) - para(4), 0);
            dz = max(abs(X(3, :)) - para(5), 0);
        
            % Calculate the external distance
            sdf_para = sqrt(dx.^2 + dy.^2 + dz.^2);
        
            % Calculate the internal distance for points inside the cuboid
            internalDist = min(max(para(3) - abs(X(1, :)), max(para(4) - abs(X(2, :)), para(5) - abs(X(3, :)))), 0);

            % Combine internal and external distances
            sdf_para = sdf_para + internalDist;
